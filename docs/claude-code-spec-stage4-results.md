...
**Run `supabase-stage4-addendum.sql` before starting.** It adds the round column and the
two rematch functions (§3).
...
**Why a round column rather than deleting votes:** a rematch must not destroy the first
round's result. The tally reads the session's current round; earlier rounds stay on
disk. It also makes the votes primary key round-aware — update it to
`(session_code, participant_name, game_id, round)`.

`participants.finished_at` must be **cleared** at the start of a new round, or
`finish_voting` will believe everyone is already done.

### Functions

**`start_rematch(p_code, p_name, p_game_ids)`**
Host only — raise `NOT_HOST` otherwise, checked against `sessions.host_name` exactly as
`force_results` does. In one transaction:
- raise `NOT_TIED` if the session status isn't `done`
- increment `sessions.round`
- replace `sessions.game_ids` with `p_game_ids` (the tied games only)
- clear `finished_at` for every participant in the session
